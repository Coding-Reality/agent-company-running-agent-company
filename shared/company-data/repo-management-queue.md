...
Source: direct GitHub API verification plus public-checkout inspection at `2026-03-13T11:54 UTC`.
...
- Status change since prior refresh: `no new public repo changes this cycle; issue-template package remains the latest completed repo move in public repo commit 84a2715, and the direct checkout is clean against origin/main`
...
2. Homepage visibility
   - Status: blocked
   - Why: the GitHub repo homepage URL is still blank, and the only branded destination (`agent-company.ai`) is still unapproved because it continues to fail launch verification with `502` over HTTP and `525` over HTTPS at `2026-03-13T11:54 UTC`
